...
# ========================================
# ADMIN TEST ENDPOINT - Simple test first
# ========================================
...
@app.get("/api/admin/processing-stats-simple", tags=["Admin"])
async def admin_stats_simple():
    """Simple admin stats endpoint"""
    try:
        async with db_manager.get_postgres_connection() as conn:
            result = await conn.fetchrow("SELECT COUNT(*) as total FROM documents WHERE embeddings IS NULL")
            return {
                "success": True,
                "unprocessed_count": result['total'],
                "message": f"Found {result['total']} unprocessed documents"
            }
    except Exception as e:
        return {
            "success": False,
...
async def ingest_document(doc: DocumentIngest, background_tasks: BackgroundTasks):
    """Ingest new documentation into knowledge base with FULL automatic processing"""
    try:
        ...
        # Prepare document data for pipeline processing
        doc_data = {
            "title": doc.title,
            "content": doc.content,
            "project": doc.project,
        ...
        # Use the enhanced pipeline processing (runs in background)
        from app.core.enhanced_ingestion import process_document_with_pipeline
        background_tasks.add_task(process_document_with_pipeline, doc_data, document_id)
        
        return {
            "status": "accepted",
            "message": f"Document '{doc.title}' queued for FULL automatic processing",
            "document_id": document_id,
            "project": doc.project,
            "embeddings_generated": True,  # Will be generated by pipeline
            "local_processing": True,
            "pipeline_processing": True,  # New flag to indicate full pipeline
            "features": {
...
# ========================================
# DATABASE STATISTICS API - Comprehensive Stats for Frontend
# ========================================

@app.get("/api/stats/database", tags=["Statistics"])
async def get_database_statistics():
    """Get comprehensive database statistics for Admin Control Center"""
    try:
        logger.info("Fetching database statistics for Admin Control Center")
        ...
        async with db_manager.get_postgres_connection() as conn:
            ...
            doc_stats = await conn.fetchrow("""
                SELECT 
                    COUNT(*) as total,
                    COUNT(*) FILTER (WHERE embeddings IS NOT NULL) as processed,
            ...
            vector_stats = await conn.fetchrow("""
                SELECT 
                    COUNT(*) as total,
                    1024 as dimensions,
                    COUNT(DISTINCT project) as collections
                FROM documents 
            ...
            session_stats = await conn.fetchrow("""
                ...
                FROM ai_god_mode_sessions
            """)
            
            # Get entity statistics (if Neo4j is available)
            entity_stats = {"total": 0, "unique": 0, "relationships": 0}
            if db_manager.neo4j_driver:
                try:
                    async with db_manager.neo4j_driver.session() as neo_session:
                        ...
                        if record:
                            entity_stats = {
                                ...
                                "unique": record["entity_count"],
                                "relationships": record["relationship_count"]
                            }
                except Exception as neo_error:
                    logger.warning(f"Neo4j stats fetch failed: {neo_error}")
            ...
            recent_stats = await conn.fetchrow("""
                SELECT 
                    COUNT(*) FILTER (WHERE created_at > NOW() - INTERVAL '24 hours') as last_24h_docs,
                    COUNT(*) FILTER (WHERE created_at > NOW() - INTERVAL '1 hour') as last_hour_docs
                FROM documents
            """)
            ...
            return {
                ...
                "data": {
                    "documents": {
                        ...
                        "unprocessed": doc_stats["unprocessed"] or 0,
                        "error_count": doc_stats["error_count"] or 0
                    },
                    "vectors": {
                        "total": vector_stats["total"] or 0,
                        "dimensions": vector_stats["dimensions"] or 1024,
                        "collections": vector_stats["collections"] or 0
                    },
                    "sessions": {
                        "total": session_stats["total"] or 0,
                        "active": session_stats["active"] or 0,
                    ...
                    "recent_activity": {
                        "last_24h": recent_stats["last_24h_docs"] if recent_stats else 0,
                        "last_hour": recent_stats["last_hour_docs"] if recent_stats else 0
                    }
                },
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
        ...
        logger.error(f"Database statistics fetch failed: {e}")
        # Return default structure even on error
        return {
            "success": False,
            "error": str(e),
            "data": {
                "documents": {"total": 0, "processed": 0, "unprocessed": 0, "error_count": 0},
                "vectors": {"total": 0, "dimensions": 1024, "collections": 0},
                "sessions": {"total": 0, "active": 0, "completed": 0},
                "entities": {"total": 0, "unique": 0, "relationships": 0},
                "queue": {"total": 0, "pending": 0, "processing": 0, "completed": 0, "failed": 0},
                "recent_activity": {"last_24h": 0, "last_hour": 0}
            },
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
...
async def get_document(document_id: str):
    """Get single document by ID with full processing status"""
    try:
        logger.info(f"Getting document: {document_id}")
        
        # Use enhanced ingestion to get document with processing status
        from app.core.enhanced_ingestion import get_document_with_processing_status
        document = await get_document_with_processing_status(document_id)
...
@app.post("/api/docs/by-id/{document_id}/reprocess", tags=["Knowledge"])
async def reprocess_document(document_id: str, background_tasks: BackgroundTasks):
    """Force reprocessing of a document through the automatic pipeline"""
    try:
        ...
        background_tasks.add_task(processing_pipeline.process_document, document)
...
@app.get("/api/docs/processing-status", tags=["Knowledge"])
async def get_processing_status(
    ...
    try:
        logger.info(f"Getting documents by processing status: {status}")
        ...
        if status == "unprocessed":
            ...
            query_conditions.append("""
                ...
                AND metadata->>'entities_extracted' = 'true'
                AND metadata->>'relationships_created' = 'true')
            """)
...
# ========================================
# ADMIN/MAINTENANCE API - Document Processing Controls
# ========================================
...
@app.post("/api/admin/bulk-embedding", tags=["Admin"])
async def start_bulk_embedding_process(
    ...
    try:
        ...
        return {
            ...
            "estimated_time_minutes": total_batches * 2,  # Rough estimate
            "status": "processing"
        }
...
@app.get("/api/admin/processing-stats", tags=["Admin"])
    ...
    try:
        async with db_manager.get_postgres_connection() as conn:
            ...
            try:
                ...
                # Fallback if processing_queue table doesn't exist or is empty
                queue_stats = {
                    'total_queue': 0,
                    'pending': 0,
                    'processing': 0,
        ...
        return {
            "success": True,
            "document_stats": dict(stats) if stats else {},
            "queue_stats": dict(queue_stats) if queue_stats else {},
            "project_breakdown": [dict(row) for row in project_stats],
            "critical_issues": {
                "unprocessed_embeddings": stats['unprocessed_embeddings'] if stats else 0,
                "queue_backlog": queue_stats['pending'] if queue_stats else 0,
                "failed_jobs": queue_stats['failed'] if queue_stats else 0
            },
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
...
@app.post("/api/admin/queue-maintenance", tags=["Admin"])
    ...
    try:
        ...
        async with db_manager.get_postgres_connection() as conn:
            if operation == "retry_failed":
                ...
                """)
                message = f"Retried {updated} failed queue items"
                ...
                """)
                message = f"Cleared {deleted} completed queue items"
                
            elif operation == "clear_failed":
                # Clear permanently failed items (retry_count >= 3)
                deleted = await conn.execute("""
                    DELETE FROM processing_queue 
                    WHERE queue_status = 'failed' AND retry_count >= 3
                """)
                message = f"Cleared {deleted} permanently failed items"
...
async def process_bulk_embeddings(job_id: str, batch_size: int, project_filter: Optional[str], total_docs: int):
...
async def startup_event():
    ...
    # Initialize automatic document processing pipeline
    try:
        from app.core import processing_pipeline
        pipeline_initialized = await processing_pipeline.initialize()
        if pipeline_initialized:
            logger.info("🤖 Automatic document processing pipeline initialized!")
            
            # Process any unprocessed documents on startup
            processed_count = await processing_pipeline.process_unprocessed_documents(limit=5)
            if processed_count > 0:
                logger.info(f"📄 Processed {processed_count} unprocessed documents on startup")
        else:
            logger.warning("⚠️  Automatic processing pipeline initialization failed")
    except Exception as e:
        logger.error(f"❌ Failed to initialize automatic processing: {e}")
    
    # Start the background document processor for periodic processing
    try:
        ...
        if status['running']:
            logger.info(f"""
            🔄 Background Document Processor Started!
            - Will process {status['batch_size']} documents every {status['interval_minutes']} minutes
            - This will gradually work through the backlog of unprocessed documents
            """)
        else:
            logger.info("⏸️ Background processor is disabled or not running")
    except Exception as e:
...
async def shutdown_event():
    ...
    logger.info("🔚 FindersKeepers v2 shutting down...")
    
    # Stop the background processor
    try:
        from app.core.background_scheduler import background_processor
        await background_processor.stop()
        logger.info("✅ Background processor stopped cleanly")
    except Exception as e:
